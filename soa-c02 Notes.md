...
## If an EC2 instance does not have the required tags, the noncompliant instance should be terminated. What is the MOST operationally efficient solution
- Create an AWS Config rule to check if the required tags are present. If an EC2 instance is noncompliant, invoke an AWS Systems Manager Automation
	- Config rule terminate or restrict many rules

## A SysOps Admin needs to build a solution to record all S3 API activity
- Create an AWS CloudTrail trail to log data events for all S3 objects
...
## Automated daily incremental backups of any EBS volume that is marked with a Lifecycle: Production tag in one of its primary AWS accounts. The company wants to precent users from using EC2 * permissions to delete any of these production snapshots
- Associate a service control policy (SCP) with the account to deny users the ability to delete EBS snapshots. Create an Amazon EventBridge rule with a 24-hour cron schedule. Configure EBS Create Snapshot as the target. Target all EBS volumes with the specified tags
	- All accounts are in AWS Organization so that the Service Control Policy would satisfy the requirements of the stipulation that deny's the deletion of a resource

## If a SysOps Admin must find the process ID of the service or process that is consuming more CPU
- Configure the Amazon CloudWatch agent procstat plugin to capture CPU process metrics
	- the procstat plugin enables you to collect metric fro individual processes. It is supported on Linux Servers and on Servers running Windows Server 2012 or later

## The company wants ensure that the website is protected fro DDoS attacks. A SysOps admin needs to deploy a solution that gives the company the ability to maintain control over the rate limit at which DDoS protections are applied 
...
## When the company needs to protect the CloudTrail log files from being modified, deleted, or forged
- Enable CloudTrail log file integrity validation
## After the release, Penetration testing revealed a cross-site scripting vulnerability that could expose user data
- AWS WAF will mitigate this issue

## an S3 bucket is hosted in the ap-southeast-2 Region, users from the eu-west-2 Region access the S3 bucket over the internet. They need faster transfers to and from the S3 bucket
- Enable S3 Transfer Acceleration on the S3 bucket. Use the New S3-accelerate endpoint's domain name for access
	- S3 Transfer acceleration uses the Amazon CloudFront global network to accelerate uploads to an downloads from an S3 bucket. This is especially useful when transferring data over long distances or across regions 
	- By enabling Transfer Acceleration, data will be routed through Amazon CloudFront's optimized network paths to S3, providing faster upload and download speeds
	- Users can use the special endpoint for Transfer Acceleration to access the bucket, and data will be automatically routed through the accelerated network

## The Lambda Functions use hardcoded credentials to connect to the database. Credentials are never stored in plaintext and the password is rotated every 30 days
- Use Secrets Manager to store credentials for the database. 
	- Secrets Manager = Password
	- AWS Secrets Manager is designed for exactly this use case, providing a secure and centralized location to store and rotate secrets such as database credentials. It can automatically rotate the password, and Lambda functions can easily retrieve the password from Secrets manager
## The application receives large traffic increases at random time throughout the day. The auto scaling group is not adding capacity fast enough. As a result, users are experiencing poor performance.
- Create a step scaling policy with settings to make larger adjustments in capacity when the system is under heavy load
	- Step scaling policies allow you to configure different scaling adjustments for different breach levels. In this case, you can set larger adjustments for larger increase in traffic. This helps the Auto Scaling group add more capacity quickly during periods of rapid traffic increases
